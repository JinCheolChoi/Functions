...
GLM_Bivariate=function(Data, Pred_Vars, Res_Var, which.family){
  ...
  for(i in 1:length(Pred_Vars)){
    #i=1
    # run model
    fullmod=as.formula( paste( Res_Var, "~", Pred_Vars[i]))
...
GEE_Bivariate_Jin=function(Data, Pred_Vars, Res_Var, ID_name, which.family="binomial"){
  ...
  for(i in 1:length(Pred_Vars)){
    ...
    target.indx=which(is.na(Data[, Pred_Vars[i]])==F) 
...
GLMM_Bivariate=function(Data,
  ...
  for(i in 1:length(Pred_Vars)){
    ...
    Coef.ind=which(grepl(Pred_Vars[i], row.names(Coef)))
    # confidence interval (exponentiated)
    CI.raw=confint(myfit, level=0.95, method="Wald")
    CI.raw.ind=which(grepl(Pred_Vars[i], row.names(CI.raw)))
    CI=exp(CI.raw)
...
CLMM_Ordinal_Bivariate_Format_1=function(Data,
  ...
  for(i in 1:length(Pred_Vars)){
    ...
    temp_out=c()
    if(Type_Odds[i]=="Prop"){
      temp_out$Estimate=round2(-Coef, 3) # put "- " in front
      temp_out$Std.Error=round2(SE.Coef, 3)
      temp_out$`P-value`=ifelse(model.fit.summ[Coef.ind, 4]<0.001, "<0.001", 
                                format(round2(model.fit.summ[Coef.ind, 4], 3), nsmall=3))
      temp_out$COR.and.CI=paste0(format(round(exp(-Coef), 2), nsmall=2), # put "- " in front
                                 " (",
                                 format(round(exp(-Raw_Upper_Bound), 2), nsmall=2), # reverse
                                 " - ",
                                 format(round(exp(-Raw_Lower_Bound), 2), nsmall=2), # reverse
                                 ")")
    }else if(Type_Odds[i]=="Non_Prop"){
      temp_out$Estimate=round2(Coef, 3)
      temp_out$Std.Error=round2(SE.Coef, 3)
...
CLMM_Ordinal_Multivariable_Format_1=function(Data,
  ...
  for(i in 1:length(Pred_Vars)){
    ...
    if(Type_Odds[i]=="Prop"){
      ...
      temp_out$Estimate=round2(-Coef[Target_Ind], 3) # put "- " in front
      temp_out$Std.Error=round2(SE.Coef[Target_Ind], 3)
      temp_out$`P-value`=ifelse(model.fit.summ[Target_Ind, 4]<0.001, "<0.001",
                                format(round2(model.fit.summ[Target_Ind, 4], 3), nsmall=3))
      temp_out$COR.and.CI=paste0(format(round(exp(-Coef[Target_Ind]), 2), nsmall=2), # put "- " in front
                                 " (",
                                 format(round(exp(-Raw_Upper_Bound[Target_Ind]), 2), nsmall=2), # reverse
                                 " - ",
                                 format(round(exp(-Raw_Lower_Bound[Target_Ind]), 2), nsmall=2), # reverse
                                 ")")
      temp_out=data.frame(temp_out)
      ...
      # output
      temp_out_summ=rbind(temp_out_summ, temp_out)
    }else if(Type_Odds[i]=="Non_Prop"){
      # output
      temp_out=c()
      ...
      temp_out$COR.and.CI=paste0(format(round(exp(Coef[Target_Ind]), 2), nsmall=2),
                                 " (",
                                 format(round(exp(Lower_Bound[Target_Ind]), 2), nsmall=2),
                                 " - ",
                                 format(round(exp(Upper_Bound[Target_Ind]), 2), nsmall=2),
                                 ")")
      temp_out=data.frame(temp_out)
  ...
  temp_out_summ=data.frame(temp_out_summ)
  for(i in 1:length(Pred_Vars)){
    #i=3
    if(Type_Odds[i]=="Prop"){
      if(is.factor(Data[, Pred_Vars[i]])){
        name_temp=expand.grid(levels(Data[, Pred_Vars[i]])[-1])
        rownames(temp_out_summ)[grep(Pred_Vars[i], rownames(temp_out_summ))]=paste0(Pred_Vars[i], " ", unlist(name_temp))
        
      }else if(is.numeric(Data[, Pred_Vars[i]])){
        rownames(temp_out_summ)[grep(Pred_Vars[i], rownames(temp_out_summ))]=paste0(Pred_Vars[i])
      }
    }else if(Type_Odds[i]=="Non_Prop"){
      # blank
    }
  ...
  output$summ_table=temp_out_summ
  return(output)
  #print(paste(i, " ", Pred_Vars[i], sep=""))
}
